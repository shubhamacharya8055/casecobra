...
import { crateCheckoutSession } from './action'
...
export default function DesignPreview({configuration}: {configuration: Configuration}) {
    ...
    const {label:modelLabel} = MODEL.options.find(({value}) => value === model)!
    ...
    const {mutate:createPaymentSession} = useMutation({
        mutationFn: crateCheckoutSession , 
        mutationKey: ["get-checkout-session"],
        onSuccess: ({url}) => {
            if(url) {
                router.push(url)
            } else {
                throw new Error("unable to retrieve the payment URL.")
            }
        },
        onError : () => {
            toast({
    ...
    >
        ...
        <div className='mt-6 sm:col-span-9 sm:mt-0 md:row-end-1'>
            ...
            <div className='sm:col-span-12 md:col-span-9 text-base'>
                <div className='grid grid-cols-1 gap-y-8 border-b border-gray-200 py-8 sm:grid-cols-2
